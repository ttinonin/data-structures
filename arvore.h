...
//Faz a inser��o dos elementos baseado no ano de nascimento dos pacientes
void inserirPorAnoABB(ABB *arv, Registro *dados){
    //inicializa uma nova celula com os dados do paciente
    EABB *novo = criaEABB(dados);
    if(arv->raiz == NULL){
        //se a raiz for null ent�o o novo elemento passa a ser a raiz
        arv->raiz = novo;
...
//Faz a inser��o dos elementos baseado no mes de nascimento dos pacientes
...
//Faz a inser��o dos elementos baseado no dia de nascimento dos pacientes
...
//Faz a inser��o dos elementos baseado na idade dos pacientes
...
//=========================FUN��ES DE REMO��O DAS ARVORES==============================================

int removerPorAno(ABB *arvore, EABB *vertice) {
  //cria um registro auxiliar para fazer as compara��es dos elementos
  Registro *v = vertice->dados;
  int ano = v->data->ano;

  //verifica se o elemento � folha
  if (vertice->filhoDir == NULL && vertice->filhoEsq == NULL) {
    ...
    if (vertice->filhoEsq != NULL && vertice->filhoDir != NULL) {
      ...
      //caso o filho esquerdo seja null, � conectado o filho direito do aux direto no vertice
    ...
    if (dados->data->ano < atual->dados->data->ano ) {
      ...
      if (dados->data->ano  > atual->dados->data->ano ) {
        ...
        if(strcmp(atual->dados->rg, dados->rg) == 0){
            //caso o rg dos dados seja igual ao rg do EABB, o EABB � retornado
            return atual;
        }
        else{
            atual = atual->filhoEsq;
...
void in_ordem(EABB *raiz) {
  ...
  if (raiz != NULL) {
        //percorre todos os elementos a esqueda, at� receber null
    ...
    printf("\n");
    //vai pegando os elementos da direita dos que est�o a esquerda
